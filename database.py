...
def listar_carteiras_do_usuario(usuario_id: int) -> list:
    ...
    try:
        cursor.execute(
            "SELECT id, nome, usuario_id FROM carteiras WHERE usuario_id = ?",
            (usuario_id,),
        )
...
def adicionar_investimento(
    carteira_id: int, nome_ativo: str, nicho: str, valor_aportado: float
):
    ...
    try:
        ...
        conexao.commit()
        print(
            f"Investimento em {nome_ativo} de R$ {valor_aportado:.2f} registrado com sucesso."
        )
    finally:
        conexao.close()
...
def buscar_nicho(carteira_id: int, nome_ativo: str) -> str | None:
    ...
    try:
        ...
        if resultado:
            return resultado[
                0
            ]  # Retorna o primeiro item da tupla, que é o nome do nicho
        return None
    finally:
        conexao.close()
